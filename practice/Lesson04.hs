...
instance Display (D4 tag) where

  display a = show (toInt a)
...
instance Display a => Display (Vec4 tag a) where

  display (Vec4 a b c d) = unwords (map display [a, b, c, d]) ++ "\n"
  

field :: Field
field = 
  let makeRow v = Vec4 v v v v
  in Vec4 
          (makeRow (Just (Red, D1)))
          (makeRow Nothing)
          (makeRow Nothing)
          (makeRow (Just (Blue, D1)))
